...
import (
	...
	bandit "github.com/EbumbaE/bandit/services/bandit-core/v3"
)
...
func main() {
	gb := bandit.NewGaussianBandit(0.5, 0.1)
	...
	for _, armID := range arms[:2] {
		memoryStorage.Save("rule1", armID, initialParams)
	}
	...
	for i := 0; i < 20_000; i++ {
		switch i {
		case 5_000:
			memoryStorage.Save("rule1", arms[2], initialParams)
		case 10_000:
			memoryStorage.Delete("rule1", arms[2])
		}

		currentArms := memoryStorage.GetAll("rule1")

		selectedArmID := gb.Select(currentArms)
